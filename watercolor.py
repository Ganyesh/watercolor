...
#-----------------Phase 1
#Readin the image
image = cv2.imread('clouds-cloudy-conifers-349732.jpg')

#resizing the image
#Interpolation is cubic for best results
image_resized = cv2.resize(image, None, fx=.25, fy=.25	)
...
image_filtered = cv2.bilateralFilter(image_cleared, 3, 10, 2)

for i in range(2):
	image_filtered = cv2.bilateralFilter(image_filtered, 3, 20, 12)

for i in range(3):
	image_filtered = cv2.bilateralFilter(image_filtered, 5, 30, 12)
